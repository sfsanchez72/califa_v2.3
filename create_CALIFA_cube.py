...
for iz in arange(nz_0,nz_1):
    ...
    for i_mc in arange(0,n_mc):
        ...
        #*(np.random.random([ny_c,nx_c])-0.5)*2
        if (iz==0):
...
#        img_loop_dec[img_loop_dec==0]=img_org[img_loop_dec==0]
        img_loop_dec = img_loop_dec*flux_ratio
        ...
        #+gaussian_filter(np.sqrt(cube_var[iz,:,:]),sigma=fsize/pix_size)*(np.random.random([ny_c,nx_c])-0.5)
